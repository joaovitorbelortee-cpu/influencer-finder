...
export default function LoginPage() {
  ...
  async function handleGoogle() {
    ...
    const callbackPath = query ? `/api/auth/callback?${query}` : "/api/auth/callback"
  ...
  return (
    <Card className="border-0 shadow-2xl">
      <CardHeader className="space-y-1">
        <CardTitle className="text-2xl font-bold text-[#1A1A2E]">Entrar</CardTitle>
      ...
      <CardContent>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <Link href={withTracking("/forgot-password")} className="text-xs text-[#6C63FF] hover:underline">
                Esqueci minha senha
              </Link>
            </div>
            <Input
        ...
        <p className="mt-4 text-center text-sm text-muted-foreground">
          Nao tem conta?{" "}
          <Link href={withTracking("/signup")} className="font-medium text-[#6C63FF] hover:underline">
            Criar conta gratis
          </Link>
        </p>
      </CardContent>
